...
Develop a MPI program to perform sum of ‘n’ numbers using point to point communication. Generate the ‘n’ numbers randomly and perform the operation between two processes using the following send functions (Standard, Ready, Synchronous) in blocking mode. Compute the time taken by each send functions and find the optimal one. [Solution File](./solution.c)
